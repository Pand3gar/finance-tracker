    ...
    >
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 overflow-y-auto px-4 py-4 sm:px-6 sm:py-5 custom-scrollbar">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            ...
            {fetching ? (
              ...
              <select
                ...
                required
                disabled={loading}
                className="h-11 w-full rounded-xl border border-white/10 bg-black/20 px-3 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-primary/50 shadow-inner appearance-none custom-select-arrow transition-colors"
              >
                <option value="" className="bg-background">-- Pilih Akun --</option>
                {accounts.map(acc => (
          ...
          {type === 'transfer' && (
            <div className="space-y-2">
              <Label htmlFor="toAccount" className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">Ke Akun</Label>
              <select
                id="toAccount"
                value={toAccountId}
                ...
                className="h-11 w-full rounded-xl border border-white/10 bg-black/20 px-3 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-primary/50 shadow-inner appearance-none custom-select-arrow transition-colors"
              >
                <option value="" className="bg-background">-- Pilih Tujuan --</option>
                {accounts
          ...
          {type !== 'transfer' && (
            <div className="space-y-2">
              <Label htmlFor="category" className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">Kategori</Label>
              {fetching ? (
                <div className="h-11 animate-pulse rounded-xl bg-muted/50" />
              ) : (
                <select
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {/* Note */}
          <div className="space-y-2">
            <Label htmlFor="note" className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">Catatan (Opsional)</Label>
            <Input
              id="note"
              type="text"
        ...
        {/* Error */}
        {error && (
          <p className="rounded-lg bg-destructive/10 px-4 py-2 text-sm text-rose-400">
            {error}
          </p>
        )}
        ...
        <div className="flex flex-col gap-3 pt-3">
          <div className="flex gap-3">
            <Button type="button" variant="outline" onClick={onClose} disabled={loading} className="flex-1 rounded-xl h-11 border-white/10 bg-white/5 hover:bg-white/10 text-foreground transition-colors">
              Batal
            </Button>
            <Button
              type="submit"
              disabled={loading || fetching}
              className={`flex-1 rounded-xl h-11 text-white font-bold transition-all shadow-md ${type === 'expense' ? 'bg-rose-500 hover:bg-rose-600 shadow-rose-500/20' :
